...
LDF <- function(xl, len1, len2, Py, mu1, mu2, sigma, l1, l2) {
  ...
  draw_plot <- function(xl, mu1, mu2, sigma, Py, l1, l2) {
    ...
    plot(xl[, 1:(n-1)], pch = 21, bg = colors[xl[,n]], col = colors[xl[,n]], 
         main = "Карта классификации нормального распределения", asp = 1, ylim=c(0, 6))
...
server <- function(input, output, session) {
  output$plot <- renderPlot({
    ...
    sigma1 <- matrix(c(input$r_sigma1, 0, 0, input$r_sigma2), 2, 2)
    ...
    mu <- rbind(m1, m2)
    sigma <- calc_sigma(first, second, m1, m2)
    print(sigma)
    P1 <- input$P
    P2 <- 1 - P1
    Py <- c(P1, P2)
    ...
    LDF(xl, len1, len2, Py, m1, m2, sigma, l1, l2)
    lines(c(mu1[1], mu2[1]), c(mu1[2], mu2[2]), col = 'magenta', lwd = 2)
  })
}
